...
static bool add_dns_cache(const char *dns, const char *srv, struct sockaddr_in *addr)
{
...
static void clear_dns_cache()
{
	for (int i = 0; i < DNS_CACHE_SIZE; i++)
	{
		...
		dns_cache[i].dns = NULL;
		if (dns_cache[i].srv)
			free(dns_cache[i].srv);
		dns_cache[i].srv = NULL;
		memset(&dns_cache[i].addr, 0, sizeof(struct sockaddr_in));
	}
}

static int resolve(const char *dns, const char *srv, struct sockaddr_in *addr)
{
